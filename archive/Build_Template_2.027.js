...
Progress:
	...
	Version 2.027
		27 January, 2017
		Temporary function changes to accomodate variable numbers of pieces per size
		Stupid 5075W pants smaller sizes don't have the same number of pieces
	...
	function nameThePieces(sorted)
	{
		try
		{
			...
			for(var a=0;a<sorted.length;a++)
			{
				...
				var newSizeLayer = prepress.layers[theSize];
		...
		try
		{
			...
			for(var a=0;a<layers.length;a++)
			{
				...
				for(var b=0;b<curLay.groupItems.length;b++)
				{
					...
					else if(thisGarment.placement[curSize][curPiece.name] == undefined)
					{
						continue;
					}
