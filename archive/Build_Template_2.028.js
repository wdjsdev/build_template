...
Progress:
	...
	Version 2.028
		01 February, 2017
		Adding the ability to switch easily from a horizontally oriented CAD file (like PS pants) or a vertically oriented CAD file (like slowpitch);
			vertically oriented files have a row for each size
			horizontally oriented files have a column for each size
		Added temp blocks for cases in which certain sizes have fewer pieces
			for example the womens full dye pants 5060W, two smallest sizes didn't have small belt loops but the rest of the sizes did.
		Temp blocks are commented
...
function container()	
{
	...
	function sortPieces(groups)
	{
		...
		//temp block to re-assign number of pieces for the smaller sizes that are lacking belt loops on 5060W full dye pants
		//temp block to re-assign number of pieces for the smaller sizes that are lacking belt loops on 5060W full dye pants
		...
		for(var a=0;a<sizeCount;a++)
		{
			...
			//temp block to re-assign number of pieces for the smaller sizes that are lacking belt loops on 5060W full dye pants
				...
				// if(a<2)
				// {
				// 	tempPieceCount = pieceCount - 2;
				// }
				// else
				// {
				// 	tempPieceCount = pieceCount;
				// }
			//temp block to re-assign number of pieces for the smaller sizes that are lacking belt loops on 5060W full dye pants
			//temp block to re-assign number of pieces for the smaller sizes that are lacking belt loops on 5060W full dye pants

			
			for(var b=0;b<tempPieceCount;b++)
			{
				temp.push(verticalSorted[b]);
			}
			verticalSorted.splice(0,tempPieceCount);
	...
	//*******************//*******************//*******************//*******************
	//*******************//*******************//*******************//*******************
	//CHANGE THIS VARIABLE IN ACCORDANCE WITH THE LAYOUT OF THE EXISTING CAD FILE
