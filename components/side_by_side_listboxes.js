function test()
{
	...
	function writeDefaultLocsFile(inUse,overflow)
	{
		if(resetGlobalDefaults)
		{
			overflow = overflowDefaults;
			inUse = inUseDefaults;
			resetGlobalDefaults = false;
		}
		var newString;
		if(overflow.length)
		{
			newString = "var overflow = [\"";
			newString += overflow.join("\", \"") + "\"];\n";
		}
		else
		{
			newString = "var overflow = [];\n\n";
		...
		{
			newString+= "var inUse = [\"";
			newString += inUse.join("\", \"") + "\"];"
		}
		else
		{
			newString = "var stdLocks = [];"
		}

		defaultLocsFile.open("w");
		defaultLocsFile.write(newString);
		defaultLocsFile.close();
